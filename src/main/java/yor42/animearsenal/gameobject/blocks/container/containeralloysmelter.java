...
public class containeralloysmelter extends Container {
    ...
    public containeralloysmelter(InventoryPlayer player, tileentitybasicalloysmelter tileentity)
    {
    ...
    public ItemStack transferStackInSlot(EntityPlayer playerIn, int index)
    {
        ...
        if(slot != null && slot.getHasStack())
        {
            ...
            else if(index != 2 && index != 1 && index != 0)
            {
                ...
                if(!AlloyRecipe.getInstance().getalloyingResult(stack1, slot1.getStack()).isEmpty())
                {
                    if(!this.mergeItemStack(stack1, 0, 2, false))
                    {
                        return ItemStack.EMPTY;
                    }
                    else if(tileentitybasicalloysmelter.isItemFuel(stack1))
                    {
